...
class SalesReturnDao {
  ...
  // =====================================================
  Future<int> saveSalesReturn(SalesReturnPaymentPayload model) async {
    final db = await dbHelper.database;
    return await db.insert('sales_return_header', {
      'sales_return_date': model.salesReturnDate,
  ...
  // =====================================================
  Future<List<Map<String, dynamic>>> getPendingSalesReturn() async {
    final db = await dbHelper.database;

    return await db.query('sales_return_header', where: 'sync_status IN (0, 2)', orderBy: 'created_at ASC');
  }

  // =====================================================
  // GET SINGLE SALES ORDER (BY LOCAL ID)
  // =====================================================
  Future<SalesReturnPaymentPayload?> getSalesOrderByLocalId(int localId) async {
    final db = await dbHelper.database;

    final result = await db.query('sales_return_header', where: 'local_id = ?', whereArgs: [localId]);
  ...
  Future<void> markSyncError(int localId, String errorMessage) async {
    ...
    await db.update('sales_return_header', {'sync_status': 2, 'sync_error': errorMessage}, where: 'local_id = ?', whereArgs: [localId]);
  }
  ...
  Future<void> deleteAfterSyncSuccess(int localId) async {
    ...
    await db.delete('sales_return_header', where: 'local_id = ?', whereArgs: [localId]);
  }

  // =====================================================
  // CLEAR ALL SALES RETURN (DEBUG / RESET)
  // =====================================================
  Future<int> clearAll() async {
    final db = await dbHelper.database;
    return await db.delete('sales_return_header');
  }

  // =====================================================
  // PRIVATE MAPPER
  // =====================================================
  SalesReturnPaymentPayload _mapRowToModel(Map<String, dynamic> row) {
