...
class QuickSalesOfflinePage extends StatefulWidget {
  const QuickSalesOfflinePage({super.key});

  @override
  State<QuickSalesOfflinePage> createState() => _QuickSalesOfflinePageState();
}

class _QuickSalesOfflinePageState extends State<QuickSalesOfflinePage> {
  final dao = QuickSalesDao();
  ...
  Future<void> loadData() async {
    final result = await dao.getPendingSalesOrder();
    setState(() {
      sales = result;
      isLoading = false;
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Quick Sales Offline"),
        actions: [IconButton(icon: const Icon(Icons.refresh), onPressed: loadData)],
      ),
      body: isLoading
          ? const Center(child: CircularProgressIndicator())
          : sales.isEmpty
          ? const Center(child: Text("Tidak ada quick sales offline 🎉"))
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _row("Sales Date", row['quick_sales_date']),
