...
class ReceiptPdfSalesOrderView extends StatefulWidget {
  const ReceiptPdfSalesOrderView({super.key, required this.salesOrderId});
  final String salesOrderId;

  @override
  State<ReceiptPdfSalesOrderView> createState() => _ReceiptPdfSalesOrderViewState();
}

class _ReceiptPdfSalesOrderViewState extends State<ReceiptPdfSalesOrderView> {
  @override
  void initState() {
    super.initState();
    Future.microtask(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBarCustom(title: 'Print Struk Sales Order'),
      body: Consumer<SalesProvider>(
        builder: (context, salesProvider, child) {
          if (salesProvider.isLoadingDetail) {
            return const Center(child: CircularProgressIndicator());
          } else if (salesProvider.salesOrderHeader == null) {
            return const Center(child: Text('Gagal memuat data sales order.'));
          } else {
            final salesOrderDetail = salesProvider.salesOrderHeader;
            return PdfPreview(
              ...
              build: (PdfPageFormat format) async {
                return _buildPdf(salesOrderDetail!, salesProvider, context);
              },
            );
          }
        },
  ...
  Future<Uint8List> _buildPdf(SalesOrderHeaderModel data, SalesProvider salesProvider, BuildContext context) async {
          ...
          return pw.DefaultTextStyle(
            ...
            child: pw.Column(
              ...
              children: [
                ...
                _row('No', data.salesOrderId),
                _row('Tanggal', DateUtilsHelper.formatYMDHM(DateTime.parse(data.salesOrderDate))),
                ...
                _calculationRow(l.order_subtotal, CurrencyFormat.toRupiah(salesProvider.subTotal()), false),
                _calculationRow(l.order_discount, CurrencyFormat.toRupiah(salesProvider.totalDiscount()), false),
                _calculationRow(l.order_grandTotalPayment, CurrencyFormat.toRupiah(salesProvider.grandTotal()), true),
                pw.SizedBox(height: 12),
                pw.SizedBox(height: 14),
                pdfDashedDivider(),
                pw.SizedBox(height: 14),
